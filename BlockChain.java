...
public class BlockChain implements Serializable {
     ...
     */
    //update pool, and update blockchain
    ...
    public boolean addBlock(Block block) {
      ...
      if (parentBlockNode==null){
        ...
        int goalHeight = parentBlockNode.height + 1;
        if (goalHeight<=maxHeightNode.height - CUT_OFF_AGE) { 
            return false;
        }

        addCoinbaseToUTXOPool(block, handler.getUTXOPool());
        BlockNode newBlockNode= new BlockNode(block, parentBlockNode, handler.getUTXOPool());
    
        blockChain.put( wrap(block.getHash()),newBlockNode);
  
        if (goalHeight > maxHeightNode.height){
